...
def load_traffic_dataset():
    dataset = []
    labels = []
    # Class_Number men 0 hatekch ll 12 maanetha 13 class aamelna bihom el classification mtaana
    for sign_type in range(CLASS_NUMBER):
        #Pour parcourir tout le dossier
        sign_list = listdir("./dataset/{}".format(sign_type))
        #pour parcourir les images de dossier num[sign_list]
        for sign_file in sign_list:
            if '.png' in sign_file:
                path = "./dataset/{}/{}".format(sign_type,sign_file)
                print(path)
                #0 pour le lecture en grayscale mode
                img = cv2.imread(path,0)
                img = cv2.resize(img, (SIZE, SIZE))
                img = np.reshape(img, [SIZE, SIZE])#returns the changed array
                #liste avec différents données
                dataset.append(img)
